class DT_intelMenu : RscDisplayTeamSwitch
{
	idd=9647;
	name="DT_intelMenu";
	movingEnable=0;
	...
	class Controls : Controls
	{
		...
		{
			idc = 1500;
			onTreeSelChanged = "_this call DT_fnc_onIntelMenuTvChange;";
		...
		class IntelInformationControlGroup: RscControlsGroup
		{
			delete HScrollBar;
			...
			class Controls
			{
				class IntelInformation: RscStructuredText
                {
                    idc = 1100;
                    text = "";
					h = 20.4 * ((((safezoneW / safezoneH) min 1.2) / 1.2) / 25);
		...
		class SelectSupportBtn : CA_ButtonContinue
		{
			idc = 2400;
			text = "Select";
			onButtonClick = "[] call DT_fnc_startIntelAction;";
		};
	};
};